...
<header>
		...
		<div class="header_menu">
			<ul>
				{#each planets as planet}
					<li>
						<a href="/{planet.id}/?tab=overview">{planet.name}</a>
					</li>
				{/each}
			</ul>
		</div>
		<button aria-expanded={isMobileMenuOpen} class="mobile-menu-button" on:click={toggleMobileMenu} type="button">
			<img alt="Menu" src="src/lib/images/Group.svg">
		</button>
	</div>
	...
	{#if isMobileMenuOpen}
		<div id="mobile-dropdown" class="mobile-dropdown">
			{#each planets as planet}
				<a href="/{planet.id}?tab=overview" on:click={handlePlanetClick}>{planet.name}</a>
			{/each}
		</div>
	{/if}
</header>
